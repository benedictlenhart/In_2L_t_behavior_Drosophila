...
#number of females at each temp?
table(phendata[sex == "female"]$temp)
...
annotation_df <- data.frame(
  ...
  start = c(0.85,  1.85, 1.85, 2.85, 2.85),
  end= c(1.15,  2, 2.15,   3, 3.15), 
  y = c(.17, .21, .225,0.265, .28),
  label = c("*","*","*","***","***")
)
...
ggsave( ((c1 + c2) + plot_layout(guides = "collect") & theme(legend.position = 'bottom')) + 
          plot_annotation(
            tag_levels = c("A")) ,
        file = "/Users/supad/OneDrive/Documents/Bergland Research/R_data_objects/Jan_2025_objects/images/srFig3.4.pdf", width = 8, height = 4)
