...
exp7data = readRDS("exp2behaviordataPROCESSED")
smol = exp7data %>% 
  select(c(flyid, genotype)) %>% 
  distinct(.)
exp6data = readRDS("exp1behaviordataPROCESSED")



#merge in modeling data for lrc
exp10data = readRDS("exp3behaviordataPROCESSED")
# exp10data %>% 
#   group_by(hour) %>% 
...
exp10fixed = exp10data %>% 
  mutate(flyid = paste0(flyid, "-", 10)) %>% 
  select(flyid, temp, intensity, inv.st, genotype,base.act,duration, scaled.act,genotype) %>% 
...
exp6fixed = exp6data %>% 
  #filter(sex == "female") %>% 
  select(-flyid) %>% 
  mutate(flyid.total = paste0(flyid.total, "-", 6)) %>% 
  ...
  rename("base.act" = "basal.act",
         "genotype" = "geno",
         "flyid" = "flyid.total") %>% 
  select(flyid, inv.st, genotype, base.act, scaled.act, duration, temp, intensity,sex,genotype) %>% 
  # mutate(temp = 25) %>% 
...
exp7fixed = exp7data %>% 
  mutate(flyid = paste0(flyid, "-", 7)) %>% 
  
  rename("base.act" = "basal.act") %>% 
  select(flyid, inv.st, temp,genotype, base.act, scaled.act, duration, temp, intensity,genotype) %>% 
...
mindata = talldata%>%
  ...
  group_by(flyidfull,temp, stimili,inv.st,sex,exp,genotype) %>%
...
saveRDS(mindata, "activity.startle.phenotypesFINAL")
...
sleeppheno = sleepdata %>% 
  ...
  rename("Value"=`Sleep Duration`) %>% 
  select(c(inv.st, temp, sex, Value, phenotype, stimili))
...
basedata = mindata %>% 
  ...
  rename("Value"="mean.base") %>% 
  select(c(inv.st, temp, sex, Value, phenotype, stimili))
phendata = rbind(sleeppheno, durpheno,magpheno,basedata)


saveRDS(phendata, "figuredataFINAL")
